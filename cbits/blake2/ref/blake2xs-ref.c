...
int _crypton_blake2xs_init( blake2xs_state *S, const size_t outlen ) {
  return _crypton_blake2xs_init_key(S, outlen, NULL, 0);
}
...
int _crypton_blake2xs_init_key( blake2xs_state *S, const size_t outlen, const void *key, size_t keylen )
{
  ...
  if( _crypton_blake2s_init_param( S->S, S->P ) < 0 ) {
    return -1;
  }
  ...
  if (keylen > 0) {
    ...
    _crypton_blake2s_update(S->S, block, BLAKE2S_BLOCKBYTES);
...
int _crypton_blake2xs_update( blake2xs_state *S, const void *in, size_t inlen ) {
  return _crypton_blake2s_update( S->S, in, inlen );
}
...
int _crypton_blake2xs_final(blake2xs_state *S, void *out, size_t outlen) {
  ...
  if (_crypton_blake2s_final(S->S, root, BLAKE2S_OUTBYTES) < 0) {
    return -1;
  }
  ...
  for (i = 0; outlen > 0; ++i) {
    ...
    _crypton_blake2s_init_param(C, P);
    /* Process key if needed */
    _crypton_blake2s_update(C, root, BLAKE2S_OUTBYTES);
    if (_crypton_blake2s_final(C, (uint8_t *)out + i * BLAKE2S_OUTBYTES, block_size) < 0) {
        return -1;
    }
    outlen -= block_size;
  }
...
int _crypton_blake2xs(void *out, size_t outlen, const void *in, size_t inlen, const void *key, size_t keylen)
{
  ...
  if (_crypton_blake2xs_init_key(S, outlen, key, keylen) < 0) {
    return -1;
  }

  /* Absorb the input message */
  _crypton_blake2xs_update(S, in, inlen);

  /* Compute the root node of the tree and the final hash using the counter construction */
  return _crypton_blake2xs_final(S, out, outlen);
}
...
int main( void )
{
  ...
  for( outlen = 1; outlen <= BLAKE2_KAT_LENGTH; ++outlen )
  {
      ...
      if( _crypton_blake2xs( hash, outlen, buf, BLAKE2_KAT_LENGTH, key, BLAKE2S_KEYBYTES ) < 0 ) {
        goto fail;
      }
  ...
  for(step = 1; step < BLAKE2S_BLOCKBYTES; ++step) {
    for (outlen = 1; outlen <= BLAKE2_KAT_LENGTH; ++outlen) {
      ...
      if( (err = _crypton_blake2xs_init_key(&S, outlen, key, BLAKE2S_KEYBYTES)) < 0 ) {
        goto fail;
      }

      while (mlen >= step) {
        if ( (err = _crypton_blake2xs_update(&S, p, step)) < 0 ) {
          goto fail;
        }
        mlen -= step;
        p += step;
      }
      if ( (err = _crypton_blake2xs_update(&S, p, mlen)) < 0) {
        goto fail;
      }
      if ( (err = _crypton_blake2xs_final(&S, hash, outlen)) < 0) {
        goto fail;
      }
