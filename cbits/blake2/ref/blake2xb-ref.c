...
int _crypton_blake2xb_init( blake2xb_state *S, const size_t outlen ) {
  return _crypton_blake2xb_init_key(S, outlen, NULL, 0);
}
...
int _crypton_blake2xb_init_key( blake2xb_state *S, const size_t outlen, const void *key, size_t keylen)
{
  ...
  if( _crypton_blake2b_init_param( S->S, S->P ) < 0 ) {
    return -1;
  }
  ...
  if (keylen > 0) {
    ...
    _crypton_blake2b_update(S->S, block, BLAKE2B_BLOCKBYTES);
...
int _crypton_blake2xb_update( blake2xb_state *S, const void *in, size_t inlen ) {
    return _crypton_blake2b_update( S->S, in, inlen );
}
...
int _crypton_blake2xb_final( blake2xb_state *S, void *out, size_t outlen) {
  ...
  if (_crypton_blake2b_final(S->S, root, BLAKE2B_OUTBYTES) < 0) {
    return -1;
  }
  ...
  for (i = 0; outlen > 0; ++i) {
    ...
    _crypton_blake2b_init_param(C, P);
    /* Process key if needed */
    _crypton_blake2b_update(C, root, BLAKE2B_OUTBYTES);
    if (_crypton_blake2b_final(C, (uint8_t *)out + i * BLAKE2B_OUTBYTES, block_size) < 0 ) {
        return -1;
    }
    outlen -= block_size;
  }
...
int _crypton_blake2xb(void *out, size_t outlen, const void *in, size_t inlen, const void *key, size_t keylen)
{
  ...
  if (_crypton_blake2xb_init_key(S, outlen, key, keylen) < 0) {
    return -1;
  }

  /* Absorb the input message */
  _crypton_blake2xb_update(S, in, inlen);

  /* Compute the root node of the tree and the final hash using the counter construction */
  return _crypton_blake2xb_final(S, out, outlen);
}
...
int main( void )
{
  ...
  for( outlen = 1; outlen <= BLAKE2_KAT_LENGTH; ++outlen )
  {
      ...
      if( _crypton_blake2xb( hash, outlen, buf, BLAKE2_KAT_LENGTH, key, BLAKE2B_KEYBYTES ) < 0 ) {
        goto fail;
      }
  ...
  for(step = 1; step < BLAKE2B_BLOCKBYTES; ++step) {
    for (outlen = 1; outlen <= BLAKE2_KAT_LENGTH; ++outlen) {
      ...
      if( (err = _crypton_blake2xb_init_key(&S, outlen, key, BLAKE2B_KEYBYTES)) < 0 ) {
        goto fail;
      }

      while (mlen >= step) {
        if ( (err = _crypton_blake2xb_update(&S, p, step)) < 0 ) {
          goto fail;
        }
        mlen -= step;
        p += step;
      }
      if ( (err = _crypton_blake2xb_update(&S, p, mlen)) < 0) {
        goto fail;
      }
      if ( (err = _crypton_blake2xb_final(&S, hash, outlen)) < 0) {
        goto fail;
      }
