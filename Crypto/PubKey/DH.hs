...
-- secrecy.  A value outside that range raises an 'error'.
--
...
-- 'Params' does not carry.
getShared :: Params -> PrivateNumber -> PublicNumber -> SharedKey
getShared (Params p _ _) (PrivateNumber x) (PublicNumber y)
    | y <= 1 || y >= p - 1 =
        error "Crypto.PubKey.DH.getShared: peer public number is out of range"
    -- the size of p, not params_bits: only p and g travel on the wire, so a
    -- caller-supplied bit size can disagree with p
    | otherwise = SharedKey $ i2ospOf_ (numBytes p) $ expSafe y x p
